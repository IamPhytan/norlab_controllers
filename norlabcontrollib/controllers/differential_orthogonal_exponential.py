import numpy as np
import util.angle_utils as angu

from norlabcontrollib.controllers import Controller
...
class DifferentialOrthogonalExponential(Controller):
    ...
    def compute_linear_velocity(self, orthogonal_projection_id):
        ...
        self.look_ahead_distance = self.path.look_ahead_distance_counter_array[
            orthogonal_projection_id
        ]

        # print("Distance to goal :" + str(self.distance_to_goal))
        # print("Path curvature :" + str(path_curvature))

        command_linear_velocity = self.maximum_linear_velocity * np.exp(
            -self.gain_path_curvature_linear * self.path_curvature
            - self.gain_distance_to_goal_linear / self.distance_to_goal
        )
        command_linear_velocity = np.clip(
            command_linear_velocity,
            self.minimum_linear_velocity,
    ...
    def compute_angular_velocity(self, state, orthogonal_projection_id):
        ...
        self.projection_pose_path_frame = self.apply_tf(
            state[:2], self.path.world_to_path_tfs_array[orthogonal_projection_id]
        )

        self.target_exponential_tangent_angle = np.arctan(
            -self.gain_path_convergence * self.projection_pose_path_frame[1]
        )

        self.robot_yaw_path_frame = angu.wrap_to_pi(
            self.current_yaw - self.path.angles[orthogonal_projection_id]
        )

        self.error_angle = (
            self.target_exponential_tangent_angle - self.robot_yaw_path_frame
        )
        ...
        command_angular_velocity = np.clip(
    ...
    def compute_command_vector(self, state):
        # print("state :", state)
        (
            self.orthogonal_projection_dist,
            self.orthogonal_projection_id,
        ) = self.path.compute_orthogonal_projection(state[:2])
        self.compute_distance_to_goal(state, self.orthogonal_projection_id)
        command_linear_velocity = self.compute_linear_velocity(
            self.orthogonal_projection_id
        )
        command_angular_velocity = self.compute_angular_velocity(
            state, self.orthogonal_projection_id
        )
